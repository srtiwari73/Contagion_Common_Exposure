...
function individual_results = compute_individual(out, Aex, Lex, Aen, Wmat, DMB, RA, dRA, lambda, Theta, S_static, N, nC, Ken, M, nL)
    ...
    % --- Initialize arrays for INDIVIDUAL BANK contributions ---
    idio = zeros(N, T_eval);
    common = zeros(N, T_eval);
    contagion = zeros(N, T_eval);
    ...
    contagion_by_price = zeros(N, T_eval);
    contagion_by_mkt = zeros(N, T_eval);
    contagion_by_shortterm = zeros(N, T_eval);
    contagion_by_lending = zeros(N, T_eval);
    ...
    for t = 1:T_eval
        ...
        idio(:,t) = Bt * Ct * idio_shocks_vector * scaling_factor * 1e4;
        ...
        common(:,t) = Bt * Ct * common_shocks_vector * scaling_factor * 1e4;
        ...
        contagion(:,t) = (Ainv - eye(N)) * Bt * Ct * ones_vector * scaling_factor * 1e4;
        ...
        contagion_by_price(:,t) = (Ainv - inv(At_no_gamma)) * Bt * Ct * ones_vector * scaling_factor * 1e4;
        ...
        contagion_by_mkt(:,t) = (Ainv - inv(At_no_delta)) * Bt * Ct * ones_vector * scaling_factor * 1e4;
        ...
        At_no_short = At + squeeze(Aen(:,1,t)) * betaG_t0(1,:);
        At_no_lend = At + squeeze(Aen(:,2,t)) * betaG_t0(2,:);
        contagion_by_shortterm(:,t) = (Ainv - inv(At_no_short)) * Bt * Ct * ones_vector * scaling_factor * 1e4;
        contagion_by_lending(:,t) = (Ainv - inv(At_no_lend)) * Bt * Ct * ones_vector * scaling_factor * 1e4;
        % for k_g = 1:Ken
    ...
    individual_results.idio = idio;
    individual_results.common = common;
    individual_results.contagion = contagion;
    individual_results.price = contagion_by_price;
    individual_results.market = contagion_by_mkt;
    individual_results.short = contagion_by_shortterm;
    individual_results.lending = contagion_by_lending;
